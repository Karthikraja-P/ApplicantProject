...
def extract_one_to_one(fpath):
    ...
    # We want ONLY those with t < 500, l < 800 to avoid sidebar/footer elements if any
    filtered = [c for c in candidates if c['top'] < 1000]
    
    mains = [c for c in filtered if not c['clickable'] and c['top'] < 100 and c['left'] < 100 and c['width'] > 200]
    opts = [c for c in filtered if c['clickable'] and c['left'] > 300]
...
def save_img(url, data, qnum, suffix):
    ...
    fname = f"iq_direct_q{qnum}_{suffix}{ext}"
...
def main():
    ...
    for slot in range(11, 16):
        ...
        js_blocks.append(f"""        {{
            ...
            answer: -1, time: 90, exp: 'Pattern logic from direct MHTML capture set.'
        }}""")

    # Edit JS
    with open(JS_PATH, 'r') as f: content = f.read()
    si = content.find('var QUESTIONS = [') + len('var QUESTIONS = [')
