document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateStep() {
        ...
        stepItems.forEach((item, i) => {
            ...
            // Disable steps beyond maxReachedStep + 1
    ...
    nextBtn.addEventListener('click', function () {
        ...
        if (!valid) {
            // Highight errors; do not proceed
            const firstBad = steps[currentStep - 1].querySelector('.error-message');
        ...
        if (currentStep < totalSteps) {
            ...
            if (currentStep > maxReachedStep) {
                ...
                localStorage.setItem('maxReachedStep', maxReachedStep);
            }
            updateStep();
        }
    });
    ...
    stepItems.forEach((item) => {
        item.addEventListener('click', function () {
            ...
            // Only allow clicking if step <= maxReachedStep
            if (step <= maxReachedStep) {
                currentStep = step;
                updateStep();
            }
    ...
    form.addEventListener('submit', function (e) {
        ...
        // NEW: Serverless Submission Flow
        submitApplication();
    });

    // ─── API Integration ────────────────────────────────────────────────────────
    const API_BASE_URL = window.API_BASE_URL || ''; // CloudFront routes /submit etc directly

    async function submitApplication() {
        const submitBtn = document.querySelector('.submit-btn');
        const statusMsg = document.createElement('div');
        statusMsg.style.cssText = 'color:#00d4ff;font-size:0.9rem;margin-top:12px;text-align:center;';
        ...
        try {
            ...
            // 1. Get Pre-signed URL for S3
            statusMsg.textContent = 'Preparing CV upload...';
            const urlGenResponse = await fetch(`${API_BASE_URL}/get-upload-url`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    filename: cvFile.name,
                    full_name: val('full_name') // Production API uses full_name for key naming
                })
            });

            if (!urlGenResponse.ok) throw new Error('Failed to get upload URL');
            const { url, key } = await urlGenResponse.json();

            // 2. Upload to S3 directly
            statusMsg.textContent = 'Uploading CV to S3...';
            ...
            if (!s3Response.ok) throw new Error('CV upload failed');

            // 3. Submit final application data
            ...
            data.cv_key = key; // Attach the S3 key
    ...
    async function saveRemoteDraft() {
        // Production backend does not support separate /save for drafts yet.
        // We rely on localStorage for draft persistence.
    ...
    setTimeout(() => {
        ...
        // if (!loaded && !isNewCandidate) fillSampleData(); // REMOVED FOR PRODUCTION
        syncPills();
        isLoading = false;
