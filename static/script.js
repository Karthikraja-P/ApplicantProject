...
document.addEventListener('DOMContentLoaded', function() {
    ...
    saveBtn.addEventListener('click', function() {
        ...
        // Also capture for local cache - convert FormData to object
        const cacheData = {};
        ...
        .catch(error => {
            ...
            const message = document.createElement('div');
            message.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #ff6f6f; color: #fff; padding: 12px 20px; border-radius: 8px; font-weight: bold; z-index: 9999;';
            message.textContent = '✗ Failed to save to server (but cached locally)';
            document.body.appendChild(message);
            setTimeout(() => message.remove(), 3000);
        });
    });
    ...
    function loadFormFromBackend() {
        console.log('ℹ Attempting to load draft from backend...');
        return fetch('/load-draft')
            .then(response => {
                console.log('ℹ Backend response status:', response.status);
                if (!response.ok) {
            ...
            .then(result => {
                console.log('ℹ Backend response:', result.status);
                if (result.status === 'success' && result.data) {
                    const data = result.data;
                    console.log('✓ Backend data received:', Object.keys(data).filter(k => data[k]).length, 'fields');
                    
                    // Restore all form fields (skip empty values)
                    ...
                    saveFormToCache();
                    console.log('✓ Backend data loaded and cached');
                    return true;
                }
                console.log('ℹ No previous draft found on backend');
                return false;
            })
            .catch(error => {
                console.log('ℹ Backend not available:', error.message);
                return false;
            });
    }
